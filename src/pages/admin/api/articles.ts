/**
 * Reads and bulk-writes the article collection. Same shape as
...
import { replaceArticles, ContentValidationError } from '../../../lib/content/repo.ts';
...
export const GET: APIRoute = ({ locals }) => {
  ...
  return json({ ok: true, data: listAllArticles() }, 200);
};
...
export const PUT: APIRoute = async ({ request, locals }) => {
  ...
  try {
    replaceArticles(body, locals.user.id);
    return json({ ok: true, data: listAllArticles() }, 200);
  } catch (error) {
    ...
    console.error('[content] failed to write articles', error);
